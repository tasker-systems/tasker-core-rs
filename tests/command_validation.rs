...
async fn test_phase1_command_serialization() {
    ...
    let command = Command::new(
        ...
        CommandPayload::RegisterWorker { worker_capabilities: capabilities.clone() },
        CommandSource::RubyWorker { id: "validation_worker".to_string() },
    );
...
async fn test_phase1_command_routing() {
    ...
    let registration_result = router.register_handler(CommandType::RegisterWorker, handler.clone()).await;
    assert!(registration_result.is_ok(), "Handler registration failed: {:?}", registration_result);
    ...
    let command = Command::new(
        ...
        CommandSource::PythonWorker { id: "routing_test_worker".to_string() },
    );

    let routing_result = router.route_command(command).await;
    assert!(routing_result.is_ok(), "Command routing failed: {:?}", routing_result);
    ...
    assert!(registered_worker.is_some(), "Worker was not registered in pool");
...
async fn test_phase1_end_to_end_workflow() {
    ...
    let register_command = Command::new(
        CommandType::RegisterWorker,
        CommandPayload::RegisterWorker { worker_capabilities: capabilities },
        CommandSource::RubyWorker { id: "e2e_worker".to_string() },
    );
