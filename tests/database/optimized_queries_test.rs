...
use tasker_core::database::{
    ActiveWorkerResult, OptimalWorkerResult, OptimizedWorkerQueries, SqlFunctionExecutor,
    WorkerHealthResult, WorkerPoolStatistics,
};
...
async fn test_sql_function_find_active_workers_for_task(pool: PgPool) {
    ...
    let _worker_named_task =
        create_test_worker_named_task(&pool, worker.worker_id, named_task.named_task_id, 100).await;
...
async fn test_optimized_queries_find_active_workers(pool: PgPool) {
    ...
    let _wnt1 =
        create_test_worker_named_task(&pool, worker1.worker_id, named_task.named_task_id, 200)
            .await;
    let _wnt2 =
        create_test_worker_named_task(&pool, worker2.worker_id, named_task.named_task_id, 100)
            .await;
...
async fn test_sql_function_get_worker_health_batch(pool: PgPool) {
    ...
    let result1 = results
        .iter()
        .find(|r| r.worker_id == worker1.worker_id)
        .unwrap();
    let result2 = results
        .iter()
        .find(|r| r.worker_id == worker2.worker_id)
        .unwrap();
    let result3 = results
        .iter()
        .find(|r| r.worker_id == worker3.worker_id)
        .unwrap();
...
async fn test_sql_function_select_optimal_worker(pool: PgPool) {
    ...
    let named_task =
        create_test_named_task(&pool, "optimal_task", namespace.task_namespace_id).await;
    ...
    let _wnt1 =
        create_test_worker_named_task(&pool, worker1.worker_id, named_task.named_task_id, 150)
            .await;
    let _wnt2 =
        create_test_worker_named_task(&pool, worker2.worker_id, named_task.named_task_id, 100)
            .await;
    ...
    assert!(
        optimal_worker.worker_name == "optimal_worker_1"
            || optimal_worker.worker_name == "optimal_worker_2"
    );
...
async fn test_optimized_queries_select_optimal_worker(pool: PgPool) {
    ...
    let named_task =
        create_test_named_task(&pool, "wrapper_task", namespace.task_namespace_id).await;
...
async fn create_test_worker_named_task(
    pool: &PgPool,
    worker_id: i32,
    named_task_id: i32,
    priority: i32,
...
async fn create_very_healthy_worker_registration(
    pool: &PgPool,
    worker_id: i32,
) -> WorkerRegistration {
    let new_registration = NewWorkerRegistration {
...
async fn create_stale_worker_registration(pool: &PgPool, worker_id: i32) -> WorkerRegistration {
    ...
    registration.last_heartbeat_at =
        Some(chrono::Utc::now().naive_utc() - chrono::Duration::minutes(5));
    registration
}
