...
mod production_validation_tests {
    ...
    async fn test_complete_linear_workflow_execution(pool: PgPool) -> sqlx::Result<()> {
        ...
        let (task_id, step_ids) = create_production_task(&pool, WorkflowPattern::Linear).await?;

        // Validate initial state
        let context = TaskExecutionContext::get_for_task(&pool, task_id)
            .await?
            .unwrap();
        ...
        for (i, &step_id) in step_ids.iter().enumerate() {
            ...
            let ready_step = readiness.iter().find(|s| s.workflow_step_id == step_id);
        ...
        // Final validation
        let final_context = TaskExecutionContext::get_for_task(&pool, task_id)
            .await?
            .unwrap();
    ...
    async fn test_diamond_workflow_parallel_execution(pool: PgPool) -> sqlx::Result<()> {
        let (task_id, step_ids) = create_production_task(&pool, WorkflowPattern::Diamond).await?;
        ...
        assert!(ready_steps
            .iter()
            .any(|s| s.workflow_step_id == step_ids[1]));
        assert!(ready_steps
            .iter()
            .any(|s| s.workflow_step_id == step_ids[2]));

        // Execute both branches in parallel (simulate)
        WorkflowStepTransitionFactory::create_complete_lifecycle(step_ids[1], &pool)
            .await
            .map_err(map_factory_error)?;
        WorkflowStepTransitionFactory::create_complete_lifecycle(step_ids[2], &pool)
            .await
            .map_err(map_factory_error)?;
        ...
        // Complete merge step
        WorkflowStepTransitionFactory::create_complete_lifecycle(step_ids[3], &pool)
            .await
            .map_err(map_factory_error)?;
    ...
    async fn test_production_error_handling_and_retry(pool: PgPool) -> sqlx::Result<()> {
        ...
        WorkflowStepTransitionFactory::create_retry_lifecycle(step_ids[1], 2, &pool)
            .await
            .map_err(map_factory_error)?;
    ...
    async fn test_complex_tree_workflow_execution(pool: PgPool) -> sqlx::Result<()> {
        let (task_id, step_ids) = create_production_task(&pool, WorkflowPattern::Tree).await?;
    ...
    async fn test_mixed_dag_workflow_complex_dependencies(pool: PgPool) -> sqlx::Result<()> {
        let (task_id, step_ids) = create_production_task(&pool, WorkflowPattern::MixedDAG).await?;
        ...
        // Complete G
        WorkflowStepTransitionFactory::create_complete_lifecycle(step_ids[6], &pool)
            .await
            .map_err(map_factory_error)?;
    ...
    async fn test_concurrent_workflow_performance(pool: PgPool) -> sqlx::Result<()> {
        ...
        for i in 0..10 {
            ...
            let (task_id, _) = create_production_task(&pool, pattern).await?;
        ...
        for task_id in &task_ids {
            let _readiness = StepReadinessStatus::get_for_task(&pool, *task_id).await?;
        }
    ...
    async fn test_state_transition_data_consistency(pool: PgPool) -> sqlx::Result<()> {
        ...
        // Retry and succeed
        WorkflowStepTransitionFactory::create_retry_lifecycle(step_ids[2], 2, &pool)
            .await
            .map_err(map_factory_error)?;
