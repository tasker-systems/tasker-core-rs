...
async fn test_circuit_breakers_enabled_from_config() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(core.circuit_breakers_enabled(), "Circuit breakers should be enabled from config");
    
    // Verify circuit breaker manager is present
    let cb_manager = core.circuit_breaker_manager();
    assert!(cb_manager.is_some(), "Circuit breaker manager should be present when enabled");
    
    info!("✅ Circuit breaker manager present: {}", cb_manager.is_some());
...
async fn test_auto_vs_explicit_config_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!("🔧 Auto-config circuit breakers enabled: {}", auto_cb_enabled);
    ...
    let explicit_core = tasker_core::orchestration::OrchestrationCore::from_config(config_manager).await?;
    let explicit_cb_enabled = explicit_core.circuit_breakers_enabled();
    info!("🛡️ Explicit config circuit breakers enabled: {}", explicit_cb_enabled);
    
    // Verify they are identical (both methods should load same config)
    assert_eq!(auto_cb_enabled, explicit_cb_enabled, 
        "Auto and explicit config modes should have identical circuit breaker settings");
    assert!(auto_cb_enabled, "Test environment should have circuit breakers enabled");
    assert!(explicit_cb_enabled, "Test environment should have circuit breakers enabled");
