...
async fn test_python_domain_event_publishing_success() -> Result<()> {
    ...
    let task_request = create_domain_event_task_request(&order_id, customer_id, amount, false);
    ...
    assert!(
        step_names.contains(&"domain_events_py_validate_order".to_string()),
        "Should have domain_events_py_validate_order step"
    );
    assert!(
        step_names.contains(&"domain_events_py_process_payment".to_string()),
        "Should have domain_events_py_process_payment step"
    );
    assert!(
        step_names.contains(&"domain_events_py_update_inventory".to_string()),
        "Should have domain_events_py_update_inventory step"
    );
    assert!(
        step_names.contains(&"domain_events_py_send_notification".to_string()),
...
async fn test_python_domain_event_publishing_concurrent() -> Result<()> {
    ...
    for i in 0..task_count {
        ...
        let task_request =
            create_domain_event_task_request(&order_id, &customer_id, 100.0 + (i as f64), false);
...
#[tokio::test]
async fn test_python_domain_event_metrics_availability() -> Result<()> {
