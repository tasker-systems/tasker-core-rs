//! Python Diamond Workflow E2E Tests
//!
...
#[tokio::test]
async fn test_python_diamond_workflow_standard() -> Result<()> {
...
#[tokio::test]
async fn test_python_diamond_workflow_parallel_branches() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    assert!(
        step_names.contains(&"diamond_start_py"),
        "Should have diamond_start_py"
    );
    assert!(
        step_names.contains(&"diamond_branch_b_py"),
        "Should have diamond_branch_b_py"
    );
    assert!(
        step_names.contains(&"diamond_branch_c_py"),
        "Should have diamond_branch_c_py"
    );
    assert!(
        step_names.contains(&"diamond_end_py"),
    ...
    println!("✅ Python diamond workflow parallel branches executed correctly");
    Ok(())
}
...
/// - End: (41 + 32) / 2 = 36.5
#[tokio::test]
async fn test_python_diamond_workflow_different_input() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    println!("✅ Python diamond workflow with input 4 completed successfully");
    Ok(())
}
