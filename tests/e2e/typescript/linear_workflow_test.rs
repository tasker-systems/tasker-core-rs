//! TypeScript Linear Workflow E2E Tests
//!
...
#[tokio::test]
async fn test_typescript_linear_workflow_standard() -> Result<()> {
...
/// - Step 4: 330 / 2 = 165
#[tokio::test]
async fn test_typescript_linear_workflow_different_input() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
...
#[tokio::test]
async fn test_typescript_linear_workflow_ordering() -> Result<()> {
    let manager = IntegrationTestManager::setup().await?;
    ...
    assert!(
        step_names.contains(&"linear_step_1"),
        "Should have linear_step_1"
    );
    assert!(
        step_names.contains(&"linear_step_2"),
        "Should have linear_step_2"
    );
    assert!(
        step_names.contains(&"linear_step_3"),
        "Should have linear_step_3"
    );
    assert!(
        step_names.contains(&"linear_step_4"),
