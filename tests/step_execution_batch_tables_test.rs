...
async fn test_step_execution_batch_tables_creation(pool: PgPool) {
    ...
    let batch_count = sqlx::query_scalar!(
        "SELECT COUNT(*) FROM tasker_step_execution_batches"
    )
    .fetch_one(&pool)
    .await
    ...
    let batch_steps_count = sqlx::query_scalar!(
        "SELECT COUNT(*) FROM tasker_step_execution_batch_steps"
    )
    .fetch_one(&pool)
    .await
    ...
    let results_count = sqlx::query_scalar!(
        "SELECT COUNT(*) FROM tasker_step_execution_batch_received_results"
    )
    .fetch_one(&pool)
    .await
    ...
    let transitions_count = sqlx::query_scalar!(
        "SELECT COUNT(*) FROM tasker_step_execution_batch_transitions"
    )
    .fetch_one(&pool)
    .await
