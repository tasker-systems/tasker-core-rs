...
impl TaskerConfig {
    ...
    pub fn get_executor_config(&self, executor_type: ExecutorType) -> ExecutorConfig {
        let executor_pools = self.executor_pools();

        let instance_config = match executor_type {
            ExecutorType::TaskRequestProcessor => &executor_pools.task_request_processor,
            ExecutorType::OrchestrationLoop => &executor_pools.step_enqueuer, // OrchestrationLoop handles step enqueueing
            ExecutorType::StepResultProcessor => &executor_pools.step_result_processor,
        };
    ...
    pub fn get_executor_instance_config(
        ...
        match executor_type {
            ExecutorType::TaskRequestProcessor => executor_pools.task_request_processor.clone(),
            ExecutorType::OrchestrationLoop => executor_pools.step_enqueuer.clone(), // OrchestrationLoop handles step enqueueing
            ExecutorType::StepResultProcessor => executor_pools.step_result_processor.clone(),
        }
    }
}
