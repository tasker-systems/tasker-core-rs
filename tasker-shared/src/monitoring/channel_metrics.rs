//! # Channel Metrics and Observability
//!
...
//!
//! ```rust
//! use tasker_shared::monitoring::channel_metrics::{ChannelMonitor, ChannelMetrics};
//! use tokio::sync::mpsc;
//!
//! # tokio_test::block_on(async {
//! let (tx, rx) = mpsc::channel::<()>(1000);
//! let monitor = ChannelMonitor::new("orchestration_command", 1000);
//!
//! // Record message sent
//! monitor.record_send_success();
//!
//! // Check saturation
//! let saturation = monitor.calculate_saturation(tx.capacity());
//! if saturation > 0.8 {
//!     println!("Channel approaching capacity!");
//! }
//! # });
//! ```
...
// TAS-51: OpenTelemetry integration for metrics export
use crate::metrics::channels::{
    CHANNEL_MESSAGES_DROPPED, CHANNEL_MESSAGES_RECEIVED, CHANNEL_MESSAGES_SENT,
...
impl ChannelMonitor {
    ...
    /// Automatically records metrics to OpenTelemetry if metrics are initialized.
    pub fn record_send_success(&self) -> bool {
        let count = self.messages_sent.fetch_add(1, Ordering::Relaxed);

        // TAS-51: Record to OpenTelemetry (with low overhead)
        // Only record every N messages to reduce metric export overhead
        let metric_record_interval = (self.buffer_size / 10).clamp(100, 1000);
        if count.is_multiple_of(metric_record_interval as u64) {
            if let Some(counter) = CHANNEL_MESSAGES_SENT.get() {
                let labels = [
                    KeyValue::new("channel_name", self.channel_name.clone()),
                    KeyValue::new("component", self.component_name.clone()),
                ];
                counter.add(metric_record_interval as u64, &labels);
            }
        }
    ...
    pub fn record_receive(&self) {
        ...
        let metric_record_interval = (self.buffer_size / 10).clamp(100, 1000);
        if count.is_multiple_of(metric_record_interval as u64) {
            if let Some(counter) = CHANNEL_MESSAGES_RECEIVED.get() {
                let labels = [
                    KeyValue::new("channel_name", self.channel_name.clone()),
                    KeyValue::new("component", self.component_name.clone()),
                ];
    ...
    pub fn record_overflow_blocked(&self) {
        ...
        // TAS-51: Record to OpenTelemetry immediately (overflow is critical)
        if let Some(counter) = CHANNEL_OVERFLOW_EVENTS.get() {
            let labels = [
                KeyValue::new("channel_name", self.channel_name.clone()),
                KeyValue::new("component", self.component_name.clone()),
            ];
    ...
    pub fn record_message_dropped(&self) {
        ...
        // TAS-51: Record to OpenTelemetry immediately (dropping is critical)
        if let Some(counter) = CHANNEL_MESSAGES_DROPPED.get() {
            let labels = [
                KeyValue::new("channel_name", self.channel_name.clone()),
                KeyValue::new("component", self.component_name.clone()),
            ];
    ...
    pub fn calculate_saturation(&self, available_capacity: usize) -> f64 {
        ...
        if saturation >= 0.95 {
            warn!(
                ...
                "⚠️  CRITICAL: Channel critically full (>95%)"
            );
        } else if saturation >= 0.80 {
            warn!(
                channel = %self.channel_name,
...
mod tests {
    ...
    fn test_channel_monitor_creation() {
        let monitor = ChannelMonitor::new("orchestration_command", 1000);
        assert_eq!(monitor.channel_name(), "orchestration_command");
        assert_eq!(monitor.component_name(), "orchestration");
