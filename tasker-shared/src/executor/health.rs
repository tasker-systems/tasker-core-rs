...
impl HealthMonitor {
    ...
    /// Record a heartbeat
    pub async fn heartbeat(&self) -> TaskerResult<()> {
        let now = Instant::now();
    ...
    pub async fn current_health(&self) -> TaskerResult<ExecutorHealth> {
        let health = self.current_health.read().await;
        Ok(health.clone())
    }

    /// Mark executor as starting with specific phase
    pub async fn mark_starting(&self, phase: &str) -> TaskerResult<()> {
    ...
    pub async fn mark_stopping(&self, graceful: bool, phase: &str) -> TaskerResult<()> {
        let mut health = self.current_health.write().await;
    ...
    pub async fn force_set_health(&self, health: ExecutorHealth) -> TaskerResult<()> {
    ...
    async fn evaluate_health(&self) -> TaskerResult<()> {
        let now = Instant::now();
    ...
    async fn get_recent_metrics(&self) -> TaskerResult<Vec<PerformanceMetric>> {
        let now = Instant::now();
        let window_duration =
