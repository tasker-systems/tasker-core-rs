...
impl ExecutorPool {
    ...
    pub async fn start_all(&self) -> TaskerResult<()> {
        info!(
    ...
    #[instrument(skip(self), fields(executor_type = %self.executor_type.name()))]
    pub async fn stop_all(&self, timeout: Duration) -> TaskerResult<()> {
        info!(
    ...
    pub async fn scale_to_count(&mut self, target_count: usize) -> TaskerResult<()> {
    ...
    async fn add_executors(&mut self, count: usize) -> TaskerResult<()> {
        info!(
        ...
        for _ in 0..count {
            ...
            let executor = BaseExecutor::with_core(self.executor_type, self.core.clone())?;
    ...
    async fn remove_executors(&mut self, count: usize) -> TaskerResult<()> {
        info!(
...
impl PoolManager {
    ...
    pub async fn new(config: Arc<ConfigManager>, core: Arc<CoordinatorCore>) -> TaskerResult<Self> {
        ...
        for executor_type in ExecutorType::all() {
            let pool = ExecutorPool::new(executor_type, config.clone(), core.clone()).await?;
    ...
    /// Start all pools
    pub async fn start_all_pools(&self) -> TaskerResult<()> {
    ...
    /// Stop all pools
    pub async fn stop_all_pools(&self, timeout: Duration) -> TaskerResult<()> {
    ...
    pub async fn get_pool_metrics(&self) -> TaskerResult<HashMap<ExecutorType, PoolMetrics>> {
    ...
    pub async fn get_health_report(&self) -> TaskerResult<HealthReport> {
    ...
    pub async fn scale_up_pool(
        &mut self,
        executor_type: ExecutorType,
        count: usize,
    ) -> TaskerResult<()> {
        let pool = self.pools.get_mut(&executor_type).ok_or_else(|| {
            crate::errors::TaskerError::OrchestrationError(format!(
    ...
    pub async fn reconfigure_pools(&mut self) -> TaskerResult<()> {
    ...
    pub async fn add_executor_to_pool(&mut self, executor_type: ExecutorType) -> TaskerResult<()> {
        let pool = self.pools.get_mut(&executor_type).ok_or_else(|| {
            crate::errors::TaskerError::OrchestrationError(format!(
                "No pool found for executor type: {}",
                executor_type.name()
        ...
        if pool.executors.len() >= pool.max_executors {
            return Err(crate::errors::TaskerError::InvalidState(format!(
        ...
        let executor = BaseExecutor::with_core(executor_type, self.core.clone(), executor_config)?;
