...
use crate::config::executor::ExecutorType;
use crate::config::ConfigManager;
use crate::{TaskerError, TaskerResult};

use self::core::CoordinatorCore;
...
impl ExecutorPoolCoordinator {
    ...
    pub async fn new(
        ...
        Self::with_operational_state(
            config_manager,
            core.clone(),
            core.operational_state_manager.clone(),
        )
        .await
    }
    ...
    pub async fn with_operational_state(
        ...
        let pool_manager = Arc::new(RwLock::new(
            PoolManager::new(config_manager.clone(), core.clone()).await?,
        ));
    ...
    pub async fn start(&self) -> TaskerResult<()> {
        ...
        // TAS-37 Supplemental: Operational state is already managed by CoordinatorCore
        // No need to transition to startup here - CoordinatorCore handles it
        ...
        *running = true;
        ...
        // TAS-37 Supplemental: Operational state is already Normal from CoordinatorCore initialization
        // No need to transition again - maintaining shared state with CoordinatorCore
    ...
    pub async fn get_orchestration_health(&self) -> TaskerResult<OrchestrationSystemHealth> {
        ...
        // Use the core to get detailed component health
        let database_healthy = self.core.database_pool().acquire().await.is_ok();
