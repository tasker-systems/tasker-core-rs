...
use tasker_shared::config::executor::ExecutorType;
...
async fn test_health_monitoring_operational_state_awareness() {
    ...
    monitor
        .record_health_report_with_operational_state(health_report.clone(), Some(&state_manager))
        .await
        .unwrap();
...
async fn test_comprehensive_shutdown_simulation() {
    ...
    let simulation_scenarios = vec![
        ...
        (
            SystemOperationalState::Startup,
            create_test_health_report(3, 10),
        ), // 30% - low during startup
        (
            SystemOperationalState::Startup,
            create_test_health_report(6, 10),
        ), // 60% - improving
        (
            SystemOperationalState::Startup,
            create_test_health_report(8, 10),
        ), // 80% - ready for normal
        // Normal operation - healthy system
        (
            SystemOperationalState::Normal,
            create_test_health_report(9, 10),
        ), // 90% - healthy
        (
            SystemOperationalState::Normal,
            create_test_health_report(10, 10),
        ), // 100% - perfect
        (
            SystemOperationalState::Normal,
            create_test_health_report(8, 10),
        ...
        (
            SystemOperationalState::GracefulShutdown,
            create_test_health_report(4, 10),
        ), // 40% - more degraded
        (
            SystemOperationalState::GracefulShutdown,
            create_test_health_report(2, 10),
        ), // 20% - mostly down
        // System stopped
        (
            SystemOperationalState::Stopped,
            create_test_health_report(0, 10),
        ), // 0% - fully stopped
    ];
    ...
    for (target_state, health_report) in simulation_scenarios {
        ...
        {
            state_manager
                .transition_to(target_state.clone())
                .await
                .unwrap();
        }
        ...
        monitor
            .record_health_report_with_config(health_report, Some(&state_manager), Some(&config))
            .await
            .unwrap();
...
async fn test_edge_case_state_transitions() {
    ...
    monitor
        .record_health_report_with_config(normal_report, Some(&state_manager), Some(&config))
        .await
        .unwrap();
    ...
    monitor
        .record_health_report_with_config(emergency_report, Some(&state_manager), Some(&config))
        .await
        .unwrap();
    ...
    assert!(state_manager
        .current_state()
        .await
        .can_transition_to(&SystemOperationalState::Stopped));
    assert!(!state_manager
        .current_state()
        .await
        .can_transition_to(&SystemOperationalState::Normal));
    assert!(!state_manager
        .current_state()
        .await
        .can_transition_to(&SystemOperationalState::GracefulShutdown));
    ...
    monitor
        .record_health_report_with_config(stopped_report, Some(&state_manager), Some(&config))
        .await
        .unwrap();
    ...
    assert!(state_manager
        .current_state()
        .await
        .can_transition_to(&SystemOperationalState::Startup));
    state_manager
        .transition_to(SystemOperationalState::Startup)
        .await
        .unwrap();
...
async fn test_operational_state_config_edge_cases() {
    ...
    let startup_multiplier = state_manager
        .health_threshold_multiplier_with_config(&extreme_config)
        .await;
    ...
    let shutdown_multiplier = state_manager
        .health_threshold_multiplier_with_config(&extreme_config)
        .await;
    ...
    assert_eq!(
        extreme_config.graceful_shutdown_timeout(),
        Duration::from_secs(1)
    );
    assert_eq!(
        extreme_config.emergency_shutdown_timeout(),
        Duration::from_secs(1)
    );
}
...
async fn test_health_summary_with_operational_context() {
    ...
    let reports = vec![
        ...
        (
            create_test_health_report(5, 10),
            SystemOperationalState::GracefulShutdown,
        ),
        (
            create_test_health_report(2, 10),
    ...
    for (report, state) in reports {
        ...
        monitor
            .record_health_report_with_config(report, Some(&state_manager), Some(&config))
            .await
            .unwrap();
