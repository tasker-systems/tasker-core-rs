...
- **{{ section.name }}** (`{{ section.path }}`) — {{ section.total_parameters() }} params{% if section.documented_parameters() > 0 %} ({{ section.documented_parameters() }} documented){% endif %}
