...
use sqlx::PgPool;
use std::sync::Arc;
use tasker_worker::testing::{WorkerTestFactory, WorkerTestData};
...
#[sqlx::test(migrator = "tasker_shared::test_utils::MIGRATOR")]
async fn test_worker_multiple_steps_creation(pool: PgPool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let workflow_steps = factory.create_test_workflow_steps(
        task.task_uuid,
        step_configs,
    ).await?;
    
    println!("✅ Created {} workflow steps for task", workflow_steps.len());
    ...
    for (i, step) in workflow_steps.iter().enumerate() {
        println!("   Step {}: {} (uuid: {})", i + 1, step.workflow_step_uuid, 
...
async fn test_worker_step_message_creation(pool: PgPool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let _foundation = test_data.foundation()
        .ok_or("Test foundation should be created")?;
    ...
    let step_message = factory.create_test_step_message(
        task_uuid,
        step_uuid,
        "test_step",
