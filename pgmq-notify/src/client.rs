...
impl PgmqNotifyClient {
    ...
    pub async fn create_queue(&mut self, queue_name: &str) -> Result<()> {
        debug!("Creating queue: {} (notifications will be handled by triggers)", queue_name);
        ...
        debug!("Successfully created queue: {} (triggers active for notifications)", queue_name);
        Ok(())
    }
    ...
    where
        T: Serialize + Send + Sync,
    {
        debug!("Sending delayed message to queue: {} (delay: {}s)", queue_name, delay_seconds);
    ...
    {
        debug!("Reading {} messages from queue: {} (vt: {})", qty, queue_name, vt);
        ...
        debug!("Successfully read {} messages from queue: {}", messages.len(), queue_name);
        Ok(messages)
    }
    ...
    pub async fn delete(&mut self, queue_name: &str, msg_id: i64) -> Result<u64> {
        ...
        debug!("Successfully deleted {} messages from queue: {}", deleted, queue_name);
        Ok(deleted)
    }
    ...
    pub async fn archive(&mut self, queue_name: &str, msg_id: i64) -> Result<u64> {
        ...
        let archived = self.client
            .archive(queue_name, msg_id)
            .await
            .map_err(|e| PgmqNotifyError::Generic(e.into()))?;
        
        debug!("Successfully archived {} messages from queue: {}", archived, queue_name);
        Ok(archived)
    }
    ...
    pub async fn purge_queue(&mut self, queue_name: &str) -> Result<u64> {
        ...
        let purged = self.client
            .purge(queue_name)
            .await
            .map_err(|e| PgmqNotifyError::Generic(e.into()))?;
        
        debug!("Successfully purged {} messages from queue: {}", purged, queue_name);
        Ok(purged)
    }
    ...
    where
        T: for<'de> Deserialize<'de> + Send + Sync,
    {
        debug!("Reading specific message {} from queue: {} (vt: {})", msg_id, queue_name, vt_seconds);
        ...
        match row {
            Some(row) => {
                ...
                let message_data: T = serde_json::from_value(message_json)
                    .map_err(PgmqNotifyError::Serialization)?;
                ...
                debug!("Successfully read specific message {} from queue: {}", msg_id, queue_name);
                Ok(Some(message))
            }
            None => {
    ...
    pub async fn delete_specific_message(&mut self, queue_name: &str, msg_id: i64) -> Result<bool> {
        debug!("Deleting specific message {} from queue: {}", msg_id, queue_name);
        ...
        if deleted {
            debug!("Successfully deleted specific message {} from queue: {}", msg_id, queue_name);
        } else {
            debug!("Specific message {} was not found for deletion from queue: {}", msg_id, queue_name);
        }
    ...
    pub async fn extend_vt_specific_message(
        ...
        debug!("Extending VT for specific message {} in queue: {} (+{}s)", msg_id, queue_name, additional_vt_seconds);
        ...
        if extended {
            debug!("Successfully extended VT for specific message {} in queue: {}", msg_id, queue_name);
        } else {
            debug!("Specific message {} was not found for VT extension in queue: {}", msg_id, queue_name);
        }
...
mod tests {
    ...
    fn test_namespace_extraction() {
        ...
        assert_eq!(config.extract_namespace("unknown_format").unwrap(), "default");
    }
