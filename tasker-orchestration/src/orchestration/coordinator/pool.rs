...
use crate::orchestration::executor::traits::OrchestrationExecutor;
use crate::orchestration::OrchestrationCore;
use tasker_shared::config::{orchestration::executor::ExecutorType, ConfigManager};
...
impl PoolManager {
    ...
    pub async fn scale_up_pool(
        ...
        let pool = self.pools.get_mut(&executor_type).ok_or_else(|| {
            tasker_shared::errors::TaskerError::OrchestrationError(format!(
    ...
    pub async fn add_executor_to_pool(&mut self, executor_type: ExecutorType) -> TaskerResult<()> {
        ...
        if pool.executors.len() >= pool.max_executors {
            return Err(tasker_shared::errors::TaskerError::InvalidState(format!(
