...
use crate::orchestration::task_finalizer::TaskFinalizer;
...
use tasker_shared::types::TaskOrchestrationResult;
use tasker_shared::types::{StepResult, ViableStep};
...
pub struct WorkflowCoordinator {
    ...
    pgmq_client: Arc<tasker_shared::messaging::PgmqClient>,
...
impl WorkflowCoordinator {
    ...
    pub fn new(
        ...
        config: WorkflowCoordinatorConfig,
        config_manager: Arc<ConfigurationManager>,
        event_publisher: tasker_shared::events::publisher::EventPublisher,
        pgmq_client: Arc<tasker_shared::messaging::PgmqClient>,
    ) -> Self {
        let sql_executor =
            tasker_shared::database::sql_functions::SqlFunctionExecutor::new(pool.clone());
        let state_manager =
        ...
        let task_finalizer = TaskFinalizer::with_event_publisher(
            pool.clone(),
    ...
    pub async fn for_testing(pool: sqlx::PgPool) -> Self {
        let config = WorkflowCoordinatorConfig::default();
        let config_manager = Arc::new(ConfigurationManager::new());
        let event_publisher = tasker_shared::events::EventPublisher::new();
        let pgmq_client =
            Arc::new(tasker_shared::messaging::PgmqClient::new_with_pool(pool.clone()).await);
    ...
    async fn discover_viable_steps(&self, task_uuid: Uuid) -> OrchestrationResult<Vec<ViableStep>> {
        ...
        if !steps.is_empty() {
            ...
            let events_viable_steps: Vec<tasker_shared::events::ViableStep> = steps
                .iter()
                .map(|step| tasker_shared::events::ViableStep {
                ...
                .await
            {
                ...
                Err(e) => {
                    ...
                    let failure_result = StepResult {
                        step_uuid: step.step_uuid,
                        status: tasker_shared::types::StepStatus::Failed,
    ...
    async fn enqueue_individual_step(
        &self,
        task_uuid: Uuid,
        task: &tasker_shared::models::core::task::Task,
        task_info: &tasker_shared::models::core::task::TaskForOrchestration,
        step: &ViableStep,
        sequence: usize,
        ...
        let execution_context = tasker_shared::messaging::message::StepExecutionContext::new(
        ...
        let step_message = tasker_shared::messaging::message::StepMessage::new(
            step.step_uuid,
            task_uuid,
            namespace.clone(),
            ...
            .await
        {
            Ok(message_id) => {
                ...
                Ok(StepResult {
                    step_uuid: step.step_uuid,
                    status: tasker_shared::types::StepStatus::InProgress,
                    output: serde_json::json!({
    ...
    async fn fetch_step_dependency_results(
        &self,
        step_uuid: Uuid,
    ) -> OrchestrationResult<Vec<tasker_shared::messaging::message::StepDependencyResult>> {
        // Get the WorkflowStep from step_uuid
        let workflow_step = tasker_shared::models::core::workflow_step::WorkflowStep::find_by_id(
            &self.database_pool,
            step_uuid,
        )
        .await
        ...
        let dependency_results: Vec<tasker_shared::messaging::message::StepDependencyResult> =
            dependencies_with_names
                .into_iter()
                .map(|(dep_step, step_name)| {
                    tasker_shared::messaging::message::StepDependencyResult::new(
                        step_name,
