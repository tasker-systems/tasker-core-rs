...
impl OrchestrationLoop {
    ...
    pub async fn new(
        ...
        let config = OrchestrationLoopConfig::from_tasker_config(&tasker_config);
    ...
    pub async fn with_unified_client(
        ...
        let step_result_processor =
            StepResultProcessor::new(pool.clone(), unified_client.clone(), tasker_config).await?;
    ...
    pub async fn with_config(
        ...
        let step_result_processor =
            StepResultProcessor::new(pool.clone(), step_result_client, tasker_config).await?;
    ...
    pub async fn run_cycle(&self) -> TaskerResult<OrchestrationCycleResult> {
        ...
        let result = OrchestrationCycleResult {
            cycle_started_at,
            cycle_duration_ms,
            tasks_claimed,
            tasks_processed,
    ...
    /// Run continuous orchestration cycles with aggregate statistics
    ///
    /// This method runs indefinitely (if max_cycles is None) and accumulates aggregate statistics
    /// without storing individual cycle results to prevent memory bloat. Returns aggregate summary.
    #[instrument(skip(self), fields(orchestrator_id = %self.orchestrator_id))]
    pub async fn run_continuous(&self) -> TaskerResult<ContinuousOrchestrationSummary> {
        info!(
            orchestrator_id = %self.orchestrator_id,
            cycle_interval_ms = self.config.cycle_interval.as_millis(),
            max_cycles = ?self.config.max_cycles,
            "Starting continuous orchestration"
        );

        let mut summary = ContinuousOrchestrationSummary::new(self.orchestrator_id.clone());
        ...
        loop {
            ...
            if let Some(max_cycles) = self.config.max_cycles {
                if cycle_count >= max_cycles {
                    info!(
                        ...
                        "Reached maximum cycles, stopping continuous orchestration"
                    );
                    break;
                }
            }
            ...
            match self.run_cycle().await {
                Ok(result) => {
                    // Aggregate the result without storing it
                    summary.accumulate_cycle_result(&result);
                    cycle_count += 1;

                    // Log summary every 100 cycles to prevent log spam
                    if cycle_count % 100 == 0 {
                        info!(
                            ...
                            "Continuous orchestration progress"
                        );
                    }
                }
                Err(e) => {
                    ...
                    // Continue running despite errors - this is a continuous system
                }
            }
    ...
    fn log_performance_details(&self, result: &OrchestrationCycleResult) {
        debug!(
            orchestrator_id = %self.orchestrator_id,
    ...
    pub fn config(&self) -> &OrchestrationLoopConfig {
        &self.config
    }
    ...
    /// Get step result processor for concurrent execution
    pub fn step_result_processor(&self) -> &StepResultProcessor {
        &self.step_result_processor
    }
}
...
impl ContinuousOrchestrationSummary {
    ...
    pub fn accumulate_cycle_result(&mut self, result: &OrchestrationCycleResult) {
        self.total_cycles += 1;
