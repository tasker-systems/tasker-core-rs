...
impl TaskClaimer {
    ...
    #[instrument(skip(self), fields(orchestrator_id = %self.orchestrator_id))]
    pub async fn release_task_claim(&self, task_uuid: Uuid) -> TaskerResult<bool> {
    ...
    #[instrument(skip(self), fields(orchestrator_id = %self.orchestrator_id))]
    pub async fn extend_task_claim(&self, task_uuid: Uuid) -> TaskerResult<bool> {
        debug!(
            task_uuid = task_uuid.to_string(),
