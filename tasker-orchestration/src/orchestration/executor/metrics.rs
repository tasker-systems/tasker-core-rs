...
impl MetricsCollector {
    ...
    pub async fn record_empty_poll(&self) -> TaskerResult<()> {
        self.increment_counter(&self.empty_polls, 1).await
    }

    /// Record an error by type
    pub async fn record_error(&self, error_type: &str) -> TaskerResult<()> {
    ...
    pub async fn record_retry(&self) -> TaskerResult<()> {
        self.increment_counter(&self.total_retries, 1).await
    }

    /// Record skipped items
    pub async fn record_skipped_items(&self, count: u64) -> TaskerResult<()> {
        self.increment_counter(&self.total_items_skipped, count)
            .await
    }

    /// Update backpressure factor
    pub async fn set_backpressure_factor(&self, factor: f64) -> TaskerResult<()> {
    ...
    pub async fn set_polling_interval_ms(&self, interval_ms: u64) -> TaskerResult<()> {
    ...
    pub async fn set_active_connections(&self, count: u32) -> TaskerResult<()> {
    ...
    /// Set queue depth
    pub async fn set_queue_depth(&self, depth: u32) -> TaskerResult<()> {
    ...
    pub async fn current_metrics(&self) -> TaskerResult<ExecutorMetrics> {
        let now = Instant::now();
    ...
    pub async fn reset(&self) -> TaskerResult<()> {
    ...
    async fn increment_counter(&self, counter: &Arc<RwLock<u64>>, value: u64) -> TaskerResult<()> {
        let mut count = counter.write().await;
        *count += value;
        Ok(())
    }

    async fn set_counter(&self, counter: &Arc<RwLock<u64>>, value: u64) -> TaskerResult<()> {
        let mut count = counter.write().await;
        *count = value;
        Ok(())
    }

    async fn read_counter(&self, counter: &Arc<RwLock<u64>>) -> TaskerResult<u64> {
        let count = counter.read().await;
        Ok(*count)
    }

    async fn read_value<T: Copy>(&self, value: &Arc<RwLock<T>>) -> TaskerResult<T> {
        let val = value.read().await;
        Ok(*val)
    }

    async fn read_string(&self, value: &Arc<RwLock<String>>) -> TaskerResult<String> {
        let val = value.read().await;
        Ok(val.clone())
    }

    async fn add_to_history(&self, data_point: MetricDataPoint) -> TaskerResult<()> {
        let mut history = self.metric_history.write().await;
    ...
    async fn calculate_recent_metrics(&self) -> TaskerResult<RecentMetrics> {
        let now = Instant::now();
    ...
    async fn calculate_average_batch_size(&self) -> TaskerResult<f64> {
        let history = self.metric_history.read().await;
        if history.is_empty() {
            return Ok(0.0);
        }
    ...
    async fn calculate_processing_statistics(&self) -> TaskerResult<ProcessingStatistics> {
        {
            let history = self.metric_history.read().await;
            if history.is_empty() {
