...
use crate::orchestration::{OrchestrationCore, OrchestrationExecutor, ProcessBatchResult};
...
use tasker_shared::config::orchestration::{ExecutorConfig, ExecutorType};
...
impl BaseExecutor {
    ...
    async fn process_batch_for_type(
        ...
        orchestration_core: &Option<Arc<OrchestrationCore>>,
    ) -> TaskerResult<ProcessBatchResult> {
        match executor_type {
...
mod tests {
    ...
    async fn create_test_database_pool() -> TaskerResult<PgPool> {
        let database_url = tasker_shared::test_utils::get_test_database_url();
