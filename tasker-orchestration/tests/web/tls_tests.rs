...
async fn test_https_configuration() {
    ...
    assert!(
        ...
        "WEB_API_TLS_ENABLED should be 'true', 'false', '1', or '0', got: '{tls_enabled}'"
    );
    ...
    if tls_enabled == "true" || tls_enabled == "1" {
        assert!(
            !cert_path.is_empty(),
            "WEB_API_TLS_CERT_PATH should not be empty when TLS is enabled"
        );
        assert!(
            !key_path.is_empty(),
            "WEB_API_TLS_KEY_PATH should not be empty when TLS is enabled"
        );
        ...
        if !cert_path.is_empty() {
            assert!(
                ...
                "Certificate path should have .crt or .pem extension: {cert_path}"
            );
        }
        ...
        if !key_path.is_empty() {
            assert!(
                ...
                "Key path should have .key or .pem extension: {key_path}"
            );
        }
    }
}
