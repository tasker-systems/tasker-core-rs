...
-- ----------------------------------------------------------------------------
-- extract_queue_namespace: Extract namespace from PGMQ queue name
-- ----------------------------------------------------------------------------
...
-- pgmq_auto_add_headers_trigger: Event trigger for PGMQ queue headers
...
-- pgmq_delete_specific_message: Delete a specific message from a queue
...
-- pgmq_ensure_headers_column: Ensure queue has headers column
...
COMMENT ON FUNCTION tasker.pgmq_ensure_headers_column(text) IS 'Ensures a pgmq queue table has the headers JSONB column required by pgmq extension v1.5.1+';

-- pgmq_extend_vt_specific_message: Extend visibility timeout for a message
...
-- pgmq_notify_queue_created: Trigger for queue creation notification
...
-- pgmq_read_specific_message: Read a specific message from a queue
CREATE FUNCTION tasker.pgmq_read_specific_message(queue_name text, target_msg_id bigint, vt_seconds integer DEFAULT 30)
...
-- pgmq_send_batch_with_notify: Send batch of messages with notifications
...
-- pgmq_send_with_notify: Send single message with notifications
