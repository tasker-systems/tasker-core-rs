...
BEGIN
    ...
    IF NOT p_dry_run THEN
        -- Archive tasks in terminal states past retention period
        WITH tasks_to_archive AS (
            ...
            WHERE tt.most_recent = true
              ...
              -- Exclude tasks still in DLQ with pending investigation
              AND NOT EXISTS (
                  SELECT 1 FROM tasker_tasks_dlq dlq
                  WHERE dlq.task_uuid = t.task_uuid
                    AND dlq.resolution_status = 'pending'
        ...
        -- Archive workflow steps (if tasks were archived)
        IF v_tasks_archived > 0 THEN
            WITH archived_tasks AS (
                SELECT task_uuid FROM tasker_tasks_archive
                WHERE archived_at >= v_start_time
            )
            INSERT INTO tasker_workflow_steps_archive
            SELECT ws.*, NOW() as archived_at
            FROM tasker_workflow_steps ws
            WHERE ws.task_uuid IN (SELECT task_uuid FROM archived_tasks);
            ...
            SELECT tt.*, NOW() as archived_at
            FROM tasker_task_transitions tt
            WHERE tt.task_uuid IN (SELECT task_uuid FROM archived_tasks);

            GET DIAGNOSTICS v_transitions_archived = ROW_COUNT;

            -- Delete from main tables (cascade should handle steps and transitions)
    ...
    ELSE
        ...
        -- Estimate steps and transitions
        SELECT
            COALESCE(SUM((SELECT COUNT(*) FROM tasker_workflow_steps ws WHERE ws.task_uuid = t.task_uuid)), 0),
            COALESCE(SUM((SELECT COUNT(*) FROM tasker_task_transitions tt2 WHERE tt2.task_uuid = t.task_uuid)), 0)
        INTO v_steps_archived, v_transitions_archived
        FROM tasker_tasks t
        JOIN tasker_task_transitions tt ON tt.task_uuid = t.task_uuid
        WHERE tt.most_recent = true
          AND tt.to_state IN ('complete', 'error', 'cancelled')
...
Archives tasks in terminal states (complete, error, cancelled) that are older
than retention period. Preserves complete audit trail in archive tables.

Exclusions:
- Tasks with pending DLQ investigations (keep in main table until resolved)
- Tasks newer than retention period
- Tasks in non-terminal states

Parameters:
- p_retention_days: Age threshold for archival (default 30 days)
- p_batch_size: Maximum tasks to archive in single call (default 1000)
- p_dry_run: If true, only returns counts without archiving

Returns:
- tasks_archived: Number of tasks moved to archive
- steps_archived: Number of workflow steps archived
- transitions_archived: Number of state transitions archived
- execution_time_ms: How long the archival took';
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM pg_proc WHERE proname = 'archive_completed_tasks'
    ) THEN
        RAISE EXCEPTION 'archive_completed_tasks function not created';
    END IF;
