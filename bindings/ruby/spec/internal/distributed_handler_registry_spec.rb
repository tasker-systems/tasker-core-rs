...
RSpec.describe TaskerCore::Orchestration::DistributedHandlerRegistry do
  ...
  describe '#get_callable_for_class' do
    ...
    it 'handles legacy process method wrapper' do
      # Create a handler with legacy process method
      legacy_handler = Object.new
      def legacy_handler.process(task, sequence, step)
        { status: 'legacy_success', method: 'process' }
      end
      
      # Mock get_handler_instance to return our legacy handler
      allow(registry).to receive(:get_handler_instance).with('LegacyHandler').and_return(legacy_handler)
      
      callable = registry.get_callable_for_class('LegacyHandler')
      expect(callable).not_to be_nil
      expect(callable).to respond_to(:call)
      
      # Test that the wrapper works
      result = callable.call(nil, nil, nil)
      expect(result[:status]).to eq('legacy_success')
      expect(result[:method]).to eq('process')
    end
  end
