...
use tasker_core::logging::{log_error, log_ffi_operation, log_step_operation};
...
use tasker_core::orchestration::step_handler::{
    StepExecutionContext, StepHandler, StepHandlerExecutor, StepResult,
};
...
impl RubyStepHandler {
    ...
    async fn convert_context_to_ruby(
        &self,
        context: &StepExecutionContext,
        pool: &sqlx::PgPool,
    ) -> OrchestrationResult<(RubyTask, RubyStepSequence, RubyStep)> {
        log_step_operation(
        ...
        debug!(
            "📂 RUBY_STEP_HANDLER: Loading task from database - task_id={}",
            context.task_id
        );
        let task = Task::find_by_id(pool, context.task_id)
            .await
            .map_err(
                |e| tasker_core::orchestration::errors::OrchestrationError::DatabaseError {
        ...
        debug!(
            "✅ RUBY_STEP_HANDLER: Task loaded successfully - task_id={}, complete={}",
            task.task_id, task.complete
        );

        // Load WorkflowStep from database
        debug!(
            "📂 RUBY_STEP_HANDLER: Loading workflow step from database - step_id={}",
            context.step_id
        );
        let workflow_step = WorkflowStep::find_by_id(pool, context.step_id)
            .await
            .map_err(
                |e| tasker_core::orchestration::errors::OrchestrationError::DatabaseError {
        ...
        debug!(
            "📂 RUBY_STEP_HANDLER: Loading step dependencies - step_id={}",
            context.step_id
        );
        let dependencies = workflow_step.get_dependencies(pool).await.map_err(|e| {
            tasker_core::orchestration::errors::OrchestrationError::DatabaseError {
                operation: "get_dependencies".to_string(),
                reason: format!(
                    "Failed to get dependencies for step {}: {}",
                    context.step_id, e
                ),
            }
        })?;
        debug!(
            "✅ RUBY_STEP_HANDLER: Dependencies loaded - count={}",
            dependencies.len()
        );
        ...
        debug!(
            "✅ RUBY_STEP_HANDLER: Ruby task and step created - task_id={}, step_name={}",
            ruby_task.task_id, ruby_step.name
        );

        // Convert dependencies to RubyStep objects
        debug!(
            "🔄 RUBY_STEP_HANDLER: Converting {} dependencies to Ruby objects",
            dependencies.len()
        );
        let ruby_dependencies: Vec<RubyStep> = dependencies
            ...
            .collect();
        debug!(
            "✅ RUBY_STEP_HANDLER: Ruby dependencies created - count={}",
            ruby_dependencies.len()
        );
        ...
        debug!(
            "🔄 RUBY_STEP_HANDLER: Creating step sequence - total_steps={}, current_position={}",
            dependencies.len() + 1,
            dependencies.len()
        );
    ...
    async fn call_ruby_process(
        &self,
        ruby_task: &RubyTask,
        ruby_sequence: &RubyStepSequence,
        ruby_step: &RubyStep,
        ...
        log_ffi_operation(
            ...
            "STARTING",
            Some(&format!(
                "handler_class={}, step_name={}",
                self.handler_class, self.step_name
            )),
            Some(&format!(
                "task_id={}, step_id={}",
                ruby_task.task_id, ruby_step.workflow_step_id
            )),
        );
        ...
        let ruby = Ruby::get().map_err(|e| {
            debug!(
                "❌ RUBY_STEP_HANDLER: Failed to get Ruby interpreter - error={}",
                e
            );
        ...
        debug!(
            "🏗️  RUBY_STEP_HANDLER: Instantiating Ruby handler - class_name={}",
            self.handler_class
        );
        let handler_class: Value =
            ruby.eval(&format!("{}::new", self.handler_class))
                .map_err(|e| {
                    debug!(
                    "❌ RUBY_STEP_HANDLER: Failed to instantiate Ruby handler - class={}, error={}",
                    self.handler_class, e
                );
                    tasker_core::orchestration::errors::OrchestrationError::FfiBridgeError {
                        operation: "instantiate_handler".to_string(),
                        reason: format!(
                            "Failed to instantiate Ruby handler {}: {}",
                            self.handler_class, e
                        ),
                    }
                })?;
        debug!(
            "✅ RUBY_STEP_HANDLER: Ruby handler instantiated successfully - class={}",
            self.handler_class
        );
        ...
        let result: Value = handler_class
            .funcall("process", (task_clone, sequence_clone, step_clone))
            .map_err(|e| {
                log_error(
                    "RubyStepHandler",
                    "call_ruby_process",
                    &format!(
                        "Ruby process method failed - handler_class={}, error={}",
                        self.handler_class, e
                    ),
                    Some(&format!(
                        "task_id={}, step_id={}",
                ...
                tasker_core::orchestration::errors::OrchestrationError::FfiBridgeError {
                    ...
                    reason: format!(
                        "Failed to call process method on {}: {}",
                        self.handler_class, e
                    ),
                }
            })?;
        ...
        debug!(
            "🔄 RUBY_STEP_HANDLER: Converting Ruby result to JSON - handler_class={}",
            self.handler_class
        );
        ...
        debug!(
            "✅ RUBY_STEP_HANDLER: Ruby result converted to JSON successfully - handler_class={}",
            self.handler_class
        );
    ...
    async fn call_ruby_process_results(
        &self,
        ruby_step: &RubyStep,
        process_output: &serde_json::Value,
        initial_results: Option<&serde_json::Value>,
    ) -> OrchestrationResult<()> {
        let ruby = Ruby::get().map_err(|e| {
            tasker_core::orchestration::errors::OrchestrationError::FfiBridgeError {
                operation: "get_ruby".to_string(),
        ...
        let method_exists: bool = handler_class
            .funcall("respond_to?", ("process_results",))
            .unwrap_or(false);

        if method_exists {
            let _result: Value = handler_class
                .funcall(
                    "process_results",
                    (step_value, process_output_value, initial_results_value),
                )
                .map_err(|e| {
                    // Don't fail the step for process_results errors, just log them
                    warn!(
                        "process_results method failed on {}: {}",
                        self.handler_class, e
                    );
                    e
                })
...
impl StepHandler for RubyStepHandler {
    ...
    async fn process(
        ...
        let (ruby_task, ruby_sequence, ruby_step) =
            self.convert_context_to_ruby(context, pool).await?;
        ...
        let result = self
            .call_ruby_process(&ruby_task, &ruby_sequence, &ruby_step)
            .await?;
    ...
    async fn process_results(
        ...
        self.call_ruby_process_results(
            &ruby_step,
            result
                .output_data
                .as_ref()
...
pub fn register_ruby_step_handler_class(ruby: &Ruby, module: &RModule) -> Result<(), Error> {
    ...
    class.define_method(
        "handler_class",
        method!(RubyStepHandlerWrapper::handler_class, 0),
    )?;
