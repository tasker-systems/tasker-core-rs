...
use crate::config::{ConfigManager, CircuitBreakerConfig};
...
impl OrchestrationCore {
    ...
    pub async fn new() -> Result<Self> {
        ...
        let config_manager = crate::config::ConfigManager::load()
            .map_err(|e| TaskerError::ConfigurationError(format!("Failed to load configuration: {e}")))?;
    ...
    pub async fn from_config(config_manager: Arc<ConfigManager>) -> Result<Self> {
        ...
        info!("📊 CORE: Database URL derived from config: {} (pool_size: {})", 
        ...
        let database_pool = sqlx::postgres::PgPoolOptions::new()
            ...
            .acquire_timeout(std::time::Duration::from_secs(config.database.checkout_timeout))
            .idle_timeout(Some(std::time::Duration::from_secs(config.database.reaping_frequency)))
            .connect(&database_url)
            .await
            .map_err(|e| TaskerError::DatabaseError(format!("Failed to connect to database with config: {e}")))?;
    ...
    async fn from_pool_and_config(
        ...
        let (circuit_breaker_manager, pgmq_client): (Option<Arc<CircuitBreakerManager>>, Arc<UnifiedPgmqClient>) = 
            if let Some(cb_config) = circuit_breaker_config {
                ...
                let protected_client = ProtectedPgmqClient::new_with_pool_and_config(
                    database_pool.clone(), 
                    &cb_config
                ).await;
                (Some(manager), Arc::new(UnifiedPgmqClient::Protected(protected_client)))
            } else {
        ...
        let orchestration_loop_config = OrchestrationLoopConfig::from_config_manager(&config_manager);
        
        // Create orchestration loop with environment-configured instance ID
        let orchestrator_id = format!("orch-{}-{}", 
            config.execution.environment, 
            Uuid::new_v4().to_string().split('-').next().unwrap_or("default")
        );
        
        let orchestration_loop = Arc::new(
            if circuit_breaker_manager.is_some() {
                // Use trait-compatible method when circuit breakers are enabled
                info!("🛡️ CORE: OrchestrationLoop using circuit breaker protected client");
                OrchestrationLoop::with_trait_client(
                    database_pool.clone(),
                    pgmq_client.clone(),
                    orchestrator_id,
                ...
                // Use standard method when circuit breakers are disabled
                info!("📤 CORE: OrchestrationLoop using standard client");
                let standard_client = PgmqClient::new_with_pool(database_pool.clone()).await;
        ...
        let task_request_config = TaskRequestProcessorConfig {
            ...
            polling_interval_seconds: (config.orchestration.task_request_polling_interval_ms / 1000) as u64,
            visibility_timeout_seconds: config.orchestration.task_request_visibility_timeout_seconds as i32,
        ...
        let step_result_processor = Arc::new(StepResultProcessor::new(
            database_pool.clone(),
            pgmq_client.clone(),
        ).await?);
    ...
    pub async fn initialize_queues(&self, namespaces: &[&str]) -> Result<()> {
        ...
        self.pgmq_client
            ...
            .await
            .map_err(|e| TaskerError::MessagingError(format!("Failed to initialize queues: {e}")))?;
