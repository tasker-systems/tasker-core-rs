...
impl ExecutorHealthStateMachine {
    ...
    pub fn process_event(&mut self, event: HealthEvent) -> Result<()> {
        debug!(
    ...
    pub fn update_performance(&mut self, success_rate: f64, error_rate: f64) -> Result<()> {
    ...
    pub fn update_heartbeat(&mut self, heartbeat_timeout_seconds: u64) -> Result<()> {
        let now = current_timestamp();
