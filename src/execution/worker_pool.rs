...
use crate::execution::command::{WorkerCapabilities, StepExecutionRequest};
...
impl WorkerPool {
    ...
    pub async fn register_worker(&self, capabilities: WorkerCapabilities) -> Result<(), WorkerPoolError> {
        ...
        if workers.contains_key(&worker_id) {
            warn!("Worker {} already registered, updating capabilities", worker_id);
        }
        
        workers.insert(worker_id.clone(), worker_state);
        info!("Worker registered: {} with {} max concurrent steps", worker_id, max_capacity);
    ...
    pub async fn unregister_worker(&self, worker_id: &str) -> Result<(), WorkerPoolError> {
        ...
        match workers.remove(worker_id) {
            Some(worker_state) => {
                info!("Worker unregistered: {} (was processing {} steps)", worker_id, worker_state.current_load);
                Ok(())
            }
            None => {
                warn!("Attempted to unregister non-existent worker: {}", worker_id);
                Err(WorkerPoolError::WorkerNotFound { worker_id: worker_id.to_string() })
            }
        }
    }

    /// Update worker heartbeat and load
    pub async fn update_worker_heartbeat(&self, worker_id: &str, current_load: usize) -> Result<(), WorkerPoolError> {
        let mut workers = self.workers.write().await;
        ...
        match workers.get_mut(worker_id) {
            Some(worker_state) => {
                ...
                debug!("Heartbeat updated for worker {}: load={}", worker_id, current_load);
                Ok(())
            }
            None => {
    ...
    pub async fn select_worker_for_batch(&self, batch_request: &BatchRequest) -> Option<String> {
        ...
        let eligible_workers: Vec<(&String, &WorkerState)> = workers
            ...
            .filter(|(_, worker)| {
                ...
                let namespace_compatible = batch_request.namespace.as_ref()
    ...
    pub async fn get_workers_by_namespace(&self, namespace: &str) -> Vec<(String, WorkerState)> {
        ...
        workers
            .iter()
            .filter(|(_, worker)| worker.capabilities.supported_namespaces.contains(&namespace.to_string()))
    ...
    pub async fn get_stats(&self) -> WorkerPoolStats {
        ...
        let healthy_workers = workers.iter().filter(|(_, w)| w.is_healthy(&self.config)).count();
    ...
    fn validate_capabilities(&self, capabilities: &WorkerCapabilities) -> Result<(), WorkerPoolError> {
...
impl WorkerState {
    ...
    pub fn is_healthy(&self, config: &WorkerPoolConfig) -> bool {
        ...
        let is_heartbeat_recent = heartbeat_age <= Duration::from_millis(config.heartbeat_timeout_ms);
...
mod tests {
    ...
    async fn test_invalid_capabilities() {
        ...
        assert!(pool.register_worker(invalid_capabilities.clone()).await.is_err());
