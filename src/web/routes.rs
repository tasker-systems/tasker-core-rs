...
pub fn api_v1_routes() -> Router<AppState> {
    Router::new()
        ...
        // Workflow Steps API  
        .route("/tasks/:uuid/workflow_steps", get(handlers::steps::list_task_steps))
        .route("/tasks/:uuid/workflow_steps/:step_uuid", get(handlers::steps::get_step))
        .route("/tasks/:uuid/workflow_steps/:step_uuid", patch(handlers::steps::resolve_step_manually))
        
        // Handlers API (read-only)
        .route("/handlers", get(handlers::registry::list_namespaces))
        .route("/handlers/:namespace", get(handlers::registry::list_namespace_handlers))
        .route("/handlers/:namespace/:name", get(handlers::registry::get_handler_info))
        
        // Analytics API (read-only)
        .route("/analytics/performance", get(handlers::analytics::get_performance_metrics))
        .route("/analytics/bottlenecks", get(handlers::analytics::get_bottlenecks))
}
