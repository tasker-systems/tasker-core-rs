...
impl DatabaseMigrations {
    ...
    fn parse_migration_filename(filename: &str) -> Option<(String, String)> {
        ...
        if filename.len() < 15 { // At least 14 digits + underscore
            return None;
        }
    ...
    async fn get_applied_migrations(pool: &PgPool) -> Result<std::collections::HashSet<String>, sqlx::Error> {
    ...
    async fn run_migration(pool: &PgPool, migration_path: &str) -> Result<(), sqlx::Error> {
        if !Path::new(migration_path).exists() {
            eprintln!("Warning: Migration file {} not found, skipping", migration_path);
            return Ok(());
        }
