...
mod tests {
    ...
    fn test_circuit_breaker_config_validation() {
        ...
        let mut invalid_config = CircuitBreakerConfig::default();
        invalid_config.failure_threshold = 0;
        assert!(invalid_config.validate().is_err());
        
        // Invalid timeout
        invalid_config = CircuitBreakerConfig::default();
    ...
    fn test_global_settings_validation() {
        ...
        let mut invalid_settings = GlobalCircuitBreakerSettings::default();
        invalid_settings.max_circuit_breakers = 0;
        assert!(invalid_settings.validate().is_err());
        
        // Invalid metrics interval
        invalid_settings = GlobalCircuitBreakerSettings::default();
        invalid_settings.metrics_collection_interval = Duration::ZERO;
        assert!(invalid_settings.validate().is_err());
    }
}