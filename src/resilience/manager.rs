...
use crate::config::{CircuitBreakerConfig, CircuitBreakerComponentConfig};
...
impl CircuitBreakerManager {
    ...
    pub async fn get_circuit_breaker(&self, component_name: &str) -> Arc<CircuitBreaker> {
        ...
        let component_config = self.config.config_for_component(component_name).to_resilience_config();
    ...
    pub async fn get_component_metrics(&self, component_name: &str) -> Option<CircuitBreakerMetrics> {
        let breakers = self.circuit_breakers.read().await;
        if let Some(breaker) = breakers.get(component_name) {
    ...
    pub async fn update_component_config(&mut self, component_name: &str, config: CircuitBreakerComponentConfig) {
        self.config.component_configs.insert(component_name.to_string(), config);
...
mod tests {
    ...
    async fn test_system_metrics_aggregation() {
        ...
        assert_eq!(state_summary.get(&crate::resilience::CircuitState::Closed), Some(&3));
