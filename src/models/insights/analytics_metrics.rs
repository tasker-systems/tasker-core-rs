...
mod tests {
    ...
    #[tokio::test]
    async fn test_get_analytics_metrics() {
        ...
        let metrics = AnalyticsMetrics::get_current(pool)
            .await
            .expect("Failed to get analytics metrics");
    ...
    #[tokio::test]
    async fn test_get_analytics_since_timestamp() {
        let db = DatabaseConnection::new()
            .await
            .expect("Failed to connect to database");
        let pool = db.pool();
        ...
        let metrics = AnalyticsMetrics::get_since(pool, Some(since))
            .await
            .expect("Failed to get analytics since timestamp");
