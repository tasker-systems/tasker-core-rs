...
fn main() -> Result<()> {
    ...
    let mut loader = match UnifiedConfigLoader::new(&environment) {
        Ok(loader) => {
            info!("✅ Successfully created UnifiedConfigLoader for environment: {}", environment);
            loader
        }
        Err(e) => {
    ...
    let known_components = [
        "database", "telemetry", "task_templates", "system", "backoff", 
        "execution", "queues", "orchestration", "circuit_breakers", 
        "task_readiness", "task_claimer", "event_systems", "worker"
    ];
    ...
    match loader.load_with_validation() {
        Ok(validated_config) => {
            info!("✅ Successfully loaded ValidatedConfig with {} components", validated_config.configs.len());
            ...
            match validated_config.to_tasker_config() {
                ...
                Err(e) => {
                    error!("❌ Failed to convert ValidatedConfig to TaskerConfig: {}", e);
                    return Err(e.into());
                }
            }
        }
...
fn display_config_summary(config: &tasker_shared::config::TaskerConfig) {
    ...
    info!("  Database: {}@{}", config.database.username, config.database.host);
    info!("  Environment: {}", config.execution.environment);
    info!("  Telemetry: {}", if config.telemetry.enabled { "enabled" } else { "disabled" });
    info!("  Queue Backend: {}", config.queues.backend);
    info!("  Orchestration Mode: {}", config.orchestration.mode);
    info!("  Active Namespaces: {:?}", config.orchestration.active_namespaces);
    info!("  Max Concurrent Tasks: {}", config.execution.max_concurrent_tasks);
    info!("  Max Concurrent Steps: {}", config.execution.max_concurrent_steps);
    info!("  Database Pool: {}-{} connections", config.database.pool.min_connections, config.database.pool.max_connections);
    info!("  Circuit Breakers: {}", if config.circuit_breakers.enabled { "enabled" } else { "disabled" });
    
    if let Some(worker_config) = &config.worker {
        info!("  Worker Configuration: present (web API {})", 
              if worker_config.web.enabled { "enabled" } else { "disabled" });
    } else {
        info!("  Worker Configuration: not present");
    }
    
    info!("  Event Systems:");
    info!("    Orchestration: {} (mode: {})", 
          config.event_systems.orchestration.system_id,
          config.event_systems.orchestration.deployment_mode);
    info!("    Task Readiness: {} (mode: {})", 
          config.event_systems.task_readiness.system_id,
          config.event_systems.task_readiness.deployment_mode);
    info!("    Worker: {} (mode: {})", 
          config.event_systems.worker.system_id,
          config.event_systems.worker.deployment_mode);
}
