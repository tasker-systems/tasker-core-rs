...
use tasker_core::execution::{
    GenericExecutor, TcpTransport, TcpTransportConfig, TransportType, UnixDatagramTransport,
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "TCP executor stats: running={}, bind_address={}",
    ...
    println!(
        "Unix executor stats: running={}, bind_address={}",
        stats.running, stats.bind_address
    );
    ...
    let tcp_executor2: tasker_core::execution::TcpExecutor =
        GenericExecutor::new(tcp_transport2).await?;
    ...
    let unix_executor2: tasker_core::execution::UnixDatagramExecutor =
        GenericExecutor::new(unix_transport2).await?;
...
async fn start_executor_generic<T>(
    ...
    T::Listener: 'static,
{
    ...
    println!(
        "Started executor on: {}",
        executor.transport().config().bind_address()
    );
    Ok(())
}
