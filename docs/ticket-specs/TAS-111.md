...
1. **Missing Makefile.toml files** for Rust crates (pgmq-notify, tasker-client, tasker-shared, tasker-orchestration, tasker-worker) and workers/rust
...
#### Library Crate Template (pgmq-notify, tasker-client, tasker-shared)
```toml
[config]
default_to_workspace = false
...
2. Create `pgmq-notify/Makefile.toml`
